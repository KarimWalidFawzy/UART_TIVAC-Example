...
void vMainAssertCalled( const char *pcFileName, unsigned int ulLineNumber )
	{
		// A breakpoint can be set on the following line to catch assertion failures
    // in the debugger. The file and line number are passed in.
    
    taskDISABLE_INTERRUPTS(); // Disable interrupts to prevent other tasks from running
    for( ;; ) {
        // This infinite loop halts the program.
        // You could also add code here to toggle an LED to indicate an error state.
    }
}
/*
